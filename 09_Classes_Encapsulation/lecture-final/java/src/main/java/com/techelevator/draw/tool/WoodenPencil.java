...
public class WoodenPencil {
    ...
    private String name = "Name";		// Favorite Names for a particular instance of a wooden pencil
	 ...
	 */
	public static void setStubLength(double newStubLength) {
		if (newStubLength >= 0.0 && newStubLength <= DEFAULT_LENGTH) {
			WoodenPencil.stubLength = newStubLength;
			
			// As: Integer myInt = new Integer(4);
			// So To: WoodenPencil myPencil = new WoodenPencil()
			// Therefore, with this method the WoodenPencil stubLength 
			//    is being SET to the parameter, not myPencil's
	...
	public void setName(String name) {
		//The this keyword specifies THIS INSTANCE's name property. 
		//   This can be avoided with a distinct parameter name in the method arguments
		this.name = name.trim().toUpperCase(Locale.FRANCE);
	}
	
	
	// THE NEW INSTANCE GETTER FOR INSTANCE DULLNESS <- RICH
	...
	public WoodenPencil() {
	    ...
	    //When I instantiate myPencil = new WoodenPencilI() -> WoodenPencil(length, shape, hardness, color, sharpness) is called
	}
	...
	public WoodenPencil(double length, int shape, String hardness, Color color, double sharpness) {
		...
		this.dullness = WoodenPencil.getMaxDullness();
	}
     ...
     */
    private boolean write(double writingWear) {
    	if (this.sharpness > maxDullness) {
    ...
    public static double clayPercentage(String hardness) {
    	...
    	//     This would be called as WoodenPencil.clayPercentage("HB"), Just like we would call Integer.parseInt("1")
