...
public class Exercises {
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for(Integer num: intArray) {
			...
			Double quotient = numDouble / 2;  // divide double
			int2Double.add(quotient);  //store into divided ArrayList
		}
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		while(i < smallerLength){           // loop for # of items in listOne
			listThree.add(listOne.get(i));
			listThree.add(listTwo.get(i));
			i++;
		}
													//what if one is smaller than the other?
		if(listOne.size()> listTwo.size()) {		 //add remaining items in longer one to listThree
